...
The odds 18-29 year olds are concerned about increase use of AI in daily life are expected to be `r round(1/exp(1/0.126), 3)` ( 1 / $e^{0.126}$) times the odds of 30 - 49 year olds.
...
Therefore, $e^{0.126}$ = `r round(exp(0.126),3)` is the expected difference in the **odds** when comparing 30 - 49 year olds to 18-29 year olds.
...
```{r}
#| echo: false
predict.glm(ai_concern_full_fit, type = "response")[1:5]
```
