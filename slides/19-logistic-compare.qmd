...
high_risk_fit_reduced <- glm(high_risk ~ age + totChol +
                                   currentSmoker,
                                 data = heart_disease, family = "binomial")
```
...
high_risk_fit_full <- glm(high_risk ~ age + totChol + currentSmoker + education, 
              data = heart_disease, family = "binomial")
```
...
**Akaike's Information Criterion (AIC)**: $$AIC = -2\log L + 2(p+1)$$ <br>

. . .

**Schwarz's Bayesian Information Criterion (BIC)**: $$ BIC = -2\log L + \log(n)\times(p+1)$$
