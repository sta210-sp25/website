...
-   Then the logistic regression model is
    ...
    $$
    \log\bigg(\frac{\hat{p}_{i1}}{1- \hat{p}_{i1}}\bigg) = \log\bigg(\frac{\hat{p}_{i1}}{\hat{p}_{i0}}\bigg) = \hat{\beta}_0 + \hat{\beta}_1 x_i + \dots \hat{\beta}_px_p
    $$

-   **Slope**, $\hat{\beta}_j$: When $x$ increases by one unit, the odds of $y=1$ versus the baseline $y=0$ are expected to multiply by a factor of $\exp\{\hat{\beta}_k\}$
...
-   Let "A" be the baseline category. Then
    ...
    \log\bigg(\frac{p_{iC}}{p_{iA}}\bigg) &= \beta_{0C} + \beta_{1C} x_{i1} + \beta_{pC}x_{ip}
    \end{aligned}
    $$
...
**Test Statistic**: $$z = \frac{\hat{\beta}_{jk} - 0}{SE(\hat{\beta}_{jk})}$$
...
::: incremental
-   We can calculate the **C% confidence interval** for $\beta_{jk}$ using $\hat{\beta}_{jk} \pm z^* \times SE(\hat{\beta}_{jk})$, where $z^*$ is calculated from the $N(0,1)$ distribution.

-   **Interpretation:** We are $C\%$ confident that for every one unit change in $x_{j}$, the odds of $y = k$ versus the baseline will multiply by a factor of $\exp\{\hat{\beta}_{jk} - z^* \times SE(\hat{\beta}_{jk})\}$ to $\exp\{\hat{\beta}_{jk} + z^* \times SE(\hat{\beta}_{jk})\}$, holding all else constant.
:::
...
::: question
Interpret the 95% confidence interval for `PhysActiveYes` in the context of the data.
:::
